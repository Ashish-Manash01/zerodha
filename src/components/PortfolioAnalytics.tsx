...
export const PortfolioAnalytics: React.FC = () => {
  ...
  holdings.forEach((holding) => {
    ...
    if (['TCS', 'INFY', 'WIPRO'].includes(holding.symbol)) {
      sectors['IT Services'] += value;
    } else if (['HDFC', 'ICICI', 'SBIN', 'AXISBANK', 'KOTAKBANK'].includes(holding.symbol)) {
      sectors['Banking'] += value;
    } else if (['MARUTI', 'BAJAJ-AUTO'].includes(holding.symbol)) {
      sectors['Automotive'] += value;
    } else if (['SUNPHARMA'].includes(holding.symbol)) {
      sectors['Pharma'] += value;
    } else if (['LT', 'ULTRACEMCO'].includes(holding.symbol)) {
      sectors['Manufacturing'] += value;
    } else if (['BHARATIARTL'].includes(holding.symbol)) {
      sectors['Telecom'] += value;
    } else if (['RELIANCE', 'POWERGRID'].includes(holding.symbol)) {
      sectors['Energy'] += value;
    } else if (['ASIANPAINT', 'NESTLEIND', 'TITAN', 'ITC'].includes(holding.symbol)) {
